sap.ui.controller("smpdemo.OpportunityMaster", {
...
*/
	onBeforeRendering: function() {
		...
		//Get list Items
		list = sap.ui.getCore().byId("opportunityListItems");
...
//	}
	handleCustomerSearch: function (evt) {
		...
		var list = sap.ui.getCore().byId("opportunityListItems");
		...
		if (sQuery && sQuery.length > 0) {
			...
			sap.ui.getCore().setModel(initialCustomerListModel, "opportunityListModel");
	...
	onCustomerSelected: function(evt){
		...
		busyDialog.open();
		$.ajax({		
			url : "http://10.11.46.11:8081/com.persistent.sfdc.Trial?path=opportunity/listviews/"+caseId+"/results",
			...
			success : function(response) {	// code to run if the request succeeds; the response is passed to the function
				...
				sap.ui.getCore().setModel(oModel,"opportunityDetailViewModel");
				//Context of selected Customer
				this.nav = sap.ui.getCore().byId('idOpportunityApp').getController();
				//Navigate to Detail Page
				this.nav.to("OpportunityDetail", "");
				
				opportunityTableId = sap.ui.getCore().byId("opportunityTableId");
				opportunityDetailViewModel = sap.ui.getCore().getModel("opportunityDetailViewModel");
				opportunityTableId.setModel(opportunityDetailViewModel);
			},
